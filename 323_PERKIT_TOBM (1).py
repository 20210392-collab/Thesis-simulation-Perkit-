...
Plain-Python version of the feasibility simulation reported in the BBA
thesis "Perkit" (MNGT 323). The same model, with identical section
numbering, is available as PERKIT_TOBM_JUPYTER.ipynb; see the README for
the mapping between sections and thesis figures.

Behaviour
    Each figure opens in its own interactive window; close a window to let
    the script continue. Summary tables print to the console in
    tab-separated form. Nothing is written to disk.

Performance
    Iteration counts and DAU ranges are set to light "reviewer" values so
    the full script finishes quickly on an ordinary machine. To reproduce
    the reporting-grade numbers from the thesis, raise ITERATIONS,
    DAU_AXIS, SOBOL_N and the per-section *_ITER values (see README).
...
#
# Imports, plot style, and every tunable parameter of the model.
# Iteration counts and DAU ranges default to light "reviewer" values so
# the script runs quickly; see the README for the reporting-grade values
# used in the thesis.
# ---------------------------------------------------------------------------
...
# A. Geography — the simulated zone and the maximum pairing distance.
...
# B. Time and behaviour — patience windows, operating day, and the
#    baseline chance of finding a spot without the app.
...
# C. DAU sweep settings — which DAU points to simulate and how many
#    Monte Carlo iterations per point (more = tighter CIs, slower).
...
# D. Spatial clustering (used by the Section 4D sweep; Section 11 has
#    its own self-contained clustering machinery). N_CLUSTERS = 0 and
#    CLUSTER_FRAC = 0.0 give the pure uniform baseline.
...
# E. Temporal pattern — whether arrivals are spread evenly through the
#    day or front-loaded. 0.5 / 0.5 = uniform all day.
...
# F. Settings for the Section 4 sensitivity sweeps.
...
# excel_table() prints a tab-separated table that pastes straight into
#               a spreadsheet (pretty=True gives an aligned console view).
# ---------------------------------------------------------------------------
...
#
# run_sim() implements the discrete-event TOBM simulation: agents enter
# in time order, the greedy rule pairs each arrival with its nearest
# active counterpart within the match radius, and a bilateral exit
# mechanism retires agents whose window has lapsed.
#
# Seeker result codes in s_waits:  >= 0 app match (0 = instant,
# >0 = waited that many minutes); -3 = unassisted find (no leaver used).
# Leaver result codes in l_waits:  >= 0 matched; -1 = expired unmatched.
# ---------------------------------------------------------------------------
...
def run_sim(dau, iterations=200, zone_w=None, zone_h=None,
    ...
    All parameters default to the globals defined in Section 0.
...
# ---------------------------------------------------------------------------
# SECTION 3 — DAU sweep (computation only)
#
# Sweeps DAU_AXIS at the baseline configuration and stores per-DAU
# outcome rates in `sweep` (list) and `df` (DataFrame) for the plots in
# Section 6.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SECTION 4 — Sensitivity sweeps (computation only)
#
# One-at-a-time sweeps over zone area (4A), match radius (4B), temporal
# mismatch (4C) and clustering fraction (4D). Results feed Section 6.
# ---------------------------------------------------------------------------
...
# 4A. Area sweep — match-rate response as the zone grows.
area_results = []
...
# 4B. Radius sweep — match-rate sensitivity to the match radius.
#     Also computes the effective-overlap grid plotted in Section 6.
rad_results = []
...
# 4C. Temporal mismatch sweep — uniform arrivals vs a seekers-early /
#     leavers-late scenario.
...
# 4D. Clustering sweep — effect of concentrating users around
#     hotspots. Skipped automatically when N_CLUSTERS == 0.
clust_sweep = {}

if N_CLUSTERS == 0:
    print('Clustering sweep skipped (N_CLUSTERS = 0 in the configuration section).')
else:
...
# ---------------------------------------------------------------------------
# SECTION 5 — Diagnostics and consistency checks
#
# Unassisted-find baseline statistics, a bilateral sanity check of the
# engine (conservation, symmetry, window and radius bounds), a
# peak/off-peak split, and the DAU summary table.
# ---------------------------------------------------------------------------

SEP  = '=' * 70
SEP2 = '-' * 70

# 5A. Unassisted-find baseline statistics.
print(SEP)
...
# 5B. Bilateral sanity check — verifies internal consistency:
#     all seekers and leavers account for 100%, seeker and leaver
#     app-match counts agree, no wait exceeds its window, and no
#     match distance exceeds the radius.
print(SEP)
...
# 5C. Peak vs off-peak split — outcome quality in the early and late
#     halves of the operating day.
...
# 5D. Summary table for the Section 3 DAU sweep.
print(SEP)
...
excel_table(
    ...
    [[r['dau'], r['p_app'], r['ci_s'], r['p_lucky'], r['p_l_match'], r['ci_l'],
      r['p_s_inst'], r['p_l_inst'], r['match_dist']] for r in sweep],
    title='DAU sweep summary')


# ---------------------------------------------------------------------------
# SECTION 6 — Exploratory plots
#
# Visualises the Section 3-4 results. Supporting material; these charts
# are not numbered figures in the thesis.
# ---------------------------------------------------------------------------
...
# Five-panel overview: outcomes, cumulative match rate, role
# comparison, and match distances across the DAU sweep.
fig = plt.figure(figsize=(20, 18), facecolor='white')
gs  = fig.add_gridspec(3, 2, hspace=0.42, wspace=0.32)
fig.suptitle(
    f'Perkit — Network Effects Simulation\n'
...
# Area and radius sensitivity (two panels).
fig, axes = plt.subplots(1, 2, figsize=(16, 6), facecolor='white')
fig.suptitle('Perkit — Sensitivity: Zone Area & Match Radius', fontsize=12, fontweight='bold', y=1.02)
...
# Effective search-overlap heatmap (boundary effects of the zone).
...
fig, axes = plt.subplots(1, 2, figsize=(16, 6), facecolor='white')
fig.suptitle('Perkit — Temporal Mismatch: Seekers Early / Leavers Late',
...
# Clustering sweep (rendered only if clustering was run).
if clust_sweep:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 6), facecolor='white')
    fig.suptitle(f'Perkit — Geographic Clustering Sweep  (σ={CLUSTER_SIGMA} km)',
                 fontsize=11, fontweight='bold', y=1.02)
    ax_a, ax_b = axes
    ...
    print('Clustering plot skipped (N_CLUSTERS = 0 in the configuration section).')
...
excel_table(['Scenario','DAU','App%','App_CI95','Unassisted%','Leave%'], _t9,
            title='Temporal mismatch (exploratory figure)')


# ---------------------------------------------------------------------------
# SECTION 7 — Single-DAU diagnostic charts (thesis Figs. 5 and 6)
#
# Self-contained: runs its own simulation at DAU = 1000 and decomposes
# seeker and leaver outcomes, match distances, and wait-time
# distributions.
# ---------------------------------------------------------------------------
...
# --- Enlarge fonts for these figures (defaults restored at the end) ---
...
excel_table(
    ...
    title='Single-DAU outcome breakdown')
...
# ---------------------------------------------------------------------------
# SECTION 8 — Seeker parking time, with vs without the app (exploratory)
#
# Reuses the Section 7 sample if present (re-runs otherwise) and
# contrasts total time-to-park against a purely unassisted search,
# which follows a geometric distribution by construction.
# ---------------------------------------------------------------------------

# --- Reuse the Section 7 sample if available, else re-run ---
try:
    sw, n, matched_sw
except NameError:
    res        = run_sim(CHART_DAU, iterations=CHART_ITER)
...
excel_table(['Scenario','Mean_min','Median_min'],
            ...
            title='Parking time with vs without the app')
print(f'Rendered  |  DAU={CHART_DAU}  |  iters={CHART_ITER}')


# ---------------------------------------------------------------------------
# SECTION 9 — Match rate vs DAU under four unassisted-search scenarios (thesis Fig. 4)
#
# Sweeps DAU for four expected unassisted-parking-time levels
# (10 / 20 / 30 / 40 min). Shaded bands are 95% confidence intervals.
# ---------------------------------------------------------------------------
...
for (sc_name, _), color, ls in zip(SCENARIOS.items(), COLORS, LINESTYLES):
    ...
    for x_val, y_val in zip(DAU_SWEEP, y):
        ax.annotate(f"{y_val:.1f}%",
                    xy=(x_val, y_val),
                    xytext=(0, 6),  # offset so the label sits above the line
...
ax.set_title('Perkit Match Rate by DAU — Four Organic Search Scenarios', fontsize=12, pad=14)

# slightly expanded limits so the annotations are not clipped
ax.set_xlim(-100, 8500)
ax.set_ylim(0, 115)
ax.yaxis.set_major_formatter(mticker.PercentFormatter(decimals=0))

# show only the milestone DAU values as x-ticks
ax.set_xticks(MILESTONES)
# rotate so adjacent labels do not overlap
...
excel_table(['Scenario','DAU','AppMatch%','CI95'], _rows9,
            title='Four-scenario DAU sweep')


# ---------------------------------------------------------------------------
# SECTION 10 — Sobol global sensitivity analysis (thesis Fig. 3)
#
# Quantifies each parameter's contribution to the variance of the
# seeker match rate over the launch parameter space (Appendix B
# ranges). S1 = first-order (direct) effect; ST = total effect
...
SOBOL_ITER = 4      # light/reviewer value — raise to ~25 to cut MC noise
...
# ---------------------------------------------------------------------------
# SECTION 11 — Stress test 1 — spatial asymmetry under clustering (thesis Fig. 7)
#
# Ten spatiotemporal hotspots (4 h, 300 m radius) draw users away from
# the uniform background. The seeker/leaver composition inside the
# clusters is swept from full leaver dominance (100/0) to full seeker
# dominance (0/100). Every random draw is seeded through a master RNG,
# so the sweep is fully reproducible; error bars are 95% CIs.
# ---------------------------------------------------------------------------
...
# Two-panel figure: city-wide vs within-cluster match rates
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
fig.suptitle('Match Success Rates Under Clustering Conditions  (error bars = 95% CI)', fontsize=12, weight='bold', y=1.02)

# Background shading: demand-excess / balanced / supply-excess regions
REGIONS = [
...
# Panel 1: city-wide seeker match rate
...
# Panel 2: match rates within the clusters
...
excel_table(
    ...
    [[lbl.replace(chr(10),' '),
     ...
     for lbl in results],
    title='Clustering stress test')


# ---------------------------------------------------------------------------
# SECTION 12 — Bilateral wait optimization for the leaver window
#
# Sweeps the leaver broadcast duration (L_WAIT) across DAU levels and
# compares the simulated optimum of the total bilateral wait,
# E[seeker wait] + E[leaver wait], with the analytical prediction
# L_WAIT*(DAU) = WINDOW / (DAU x Pi1). Motivates the 2-minute leaver
# window used in the product design.
# ---------------------------------------------------------------------------
...
excel_table(['DAU','L_WAIT_min','SeekMatch%','Seek_CI95','LeaveMatch%','Leave_CI95',
            ...
            title='Leaver-window optimization')


# ---------------------------------------------------------------------------
# SECTION 13 — Stress test 2 — temporal asymmetry (thesis Fig. 8)
#
# Seeker arrivals are fixed at 70% before the 15:30 split; the leaver
# share before the split is varied (70 / 50 / 30 / 10%) to isolate the
# effect of supply-side timing on the platform match rate.
# ---------------------------------------------------------------------------
...
excel_table(['Seekers<15:30_%','Leavers<15:30_%','SeekerMatch%','CI95'],
            ...
            title='Temporal asymmetry stress test')


# ---------------------------------------------------------------------------
# SECTION 14 — Stress test 3 — combined stress test (thesis Fig. 9)
#
# Three adverse conditions imposed simultaneously:
#   1. population deficit: 1,000 seekers vs 800 leavers
#      (demand-led adoption);
#   2. spatial clustering: 10 Gaussian clusters (sigma = 0.10 km)
#      absorbing half of all agents at a 70/30 seeker/leaver mix;
#   3. temporal mismatch: 700 seekers but only 400 leavers active
#      before 15:30 (a 70%/50% split).
#
# Design note — clustering here is spatial only. The temporal stressor
# is already supplied, cleanly and exactly, by the 70/50 split
# (condition 3); giving the clusters their own 4-hour windows would
# stack a second temporal mechanism on the same agents and blur which
# lever drives the result. The spatiotemporal (4-hour) clusters belong
# to Section 11, where time-localised hotspots are the object of study.
#
# Cluster composition is fixed by clustered counts (630 seekers / 270
# leavers), not by equal per-role fractions, so the 70/30 mix inside
# clusters holds exactly despite the unequal population sizes.
# ---------------------------------------------------------------------------
